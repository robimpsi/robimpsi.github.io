...
Because Gunicorn was pinned to `--workers 1 --threads 1`, there was exactly one process and one thread serving requests. The singleton was safe. The Celery worker used an in-memory `:memory:` DuckDB instance and never touched the file on disk at `/data-lake/cache/nkdash.duckdb`. That file lock was owned exclusively by the Dash app. When ETL finished, a signal went through Redis, and the app reloaded its connection in a background thread.  
