...
async function generate() {
  ...
  const postsManifest = posts.map(({ content, ...rest }) => rest);
  await fs.writeFile(path.join(postsApiDir, "index.json"), JSON.stringify(postsManifest, null, 2));
  ...
  const projectsManifest = projects.map(({ content, ...rest }) => rest);
  await fs.writeFile(path.join(projectsApiDir, "index.json"), JSON.stringify(projectsManifest, null, 2));
